# GIS 学习日志 | WebGIS 开发转型记录

> 测绘遥感本科 → 烘焙师 → WebGIS 开发工程师（目标）

## 个人背景
- **学历**：2024年 地理信息科学（测绘遥感方向）本科
- **现状**：佛山烘焙师，单休，计划14个月转型
- **目标**：佛山本地 WebGIS/三维GIS 开发岗位
- **优势**：测绘遥感专业背景，熟悉 ENVI 影像处理

## 学习规划

| 阶段 | 时间 | 内容 | 状态 |
|:---|:---|:---|:---|
| Phase 01 | 2026.04-2026.07 | JavaScript + Leaflet 二维GIS | 🟡 进行中 |
| Phase 02 | 2026.08-2027.01 | Cesium 三维GIS 核心 | ⚪ 待开始 |
| Phase 03 | 2027.02-2027.06 | 项目整合 + 求职 | ⚪ 待开始 |

## 每日记录

| 日期 | 学习内容 | 时长 | 代码/笔记 |
|:---|:---|:---|:---|
| 2026.03.30 | 登录GitHub，搭建仓库 | 20min | 本页面 |
| 2026.03.31 | JavaScript变量学习 | 1h | [day02-variables.js](./Phase-01-JS-Leaflet/Week-01-02-JS-Basic/day02-variables.js) |
| 2026.04.01 | JavaScript数据类型学习 | 30min | [day03-datatypes.js](./Phase-01-JS-Leaflet/Week-01-02-JS-Basic/day03-datatypes.js) |
| 2026.04.02 | JavaScript运算符学习 | 40min | [day04-operators.js](./Phase-01-JS-Leaflet/Week-01-02-JS-Basic/day04-operators.js) |
| 2026.04.04 | JavaScript条件判断学习 | 40min | [day05-condition.js](./Phase-01-JS-Leaflet/Week-01-02-JS-Basic/day05-condition.js) |
| 2026.04.05 | JavaScript条件判断学习 | 1h | [day06-loop.js](./Phase-01-JS-Leaflet/Week-01-02-JS-Basic/day06-loop.js) |
| 2026.04.06 | JavaScript条件判断学习 | 2h | [day07-project.js](./Phase-01-JS-Leaflet/Week-01-02-JS-Basic/day07-project.js) |

## 本周目标
- [ ] 理解变量声明（var/let/const）
- [ ] 掌握基本数据类型
- [ ] 完成5个基础代码练习

## 收藏资源
- [现代JavaScript教程](https://zh.javascript.info/)
- [Leaflet官方文档](https://leafletjs.com/)
